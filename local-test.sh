...
kubectl wait --namespace cloud-ruby --for=condition=ready pod --selector=app=cloud-ruby --timeout=10s

# test that we can get to the application
curl --include --silent http://localhost:80/hello-world | grep "200 OK"